...
void print_map()
{
    char lookup[] = {'W', 'D', 'P', 'A', 'K', 'C', 'N',' ','S', 'O', 'Z', '_', 'L', 'T', 'B', 'R'};
...
MapItem* get_current(int x, int y)
{
    ...
    MapItem *mapItem = (MapItem*)getItem(get_active_map()->items, key);
    if (mapItem && mapItem->type == CLEAR) {
...
MapItem* get_north(int x, int y)
{
    ...
    if (mapItem && mapItem->type == CLEAR) {
        //pc.printf("inside clear");
        removeItem(get_active_map()->items, key);
    }
    //pc.printf("inside get_north");
    //pc.printf("%s", (mapItem == NULL) ? "true" : "false");
    return mapItem;
}
...
void map_erase(int x, int y)
{
    ...
    *item = CLEAR_SENTINEL;
    void *rem = insertItem(get_active_map()->items, XY_KEY(x,y), item);
    if (rem) {
        free(rem);
    }
    */
